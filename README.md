...
> 数据：长三角（无锡 / 苏州 / 南京 / 上海 / 常州 / 南通 / 杭州 / 宁波）岗位。当前 **2000 条**，其中真实核校样本（来源：猎聘 / 实习僧 / 智联 / 应届生求职网 / 高校就业网等）约 222 条，AI 辅助扩充约 1778 条，所有条目均带 `source` 标注以便区分。
...
- **数据看板**：页面顶部实时展示岗位总数、城市 / 类型 / **学历要求** / 方向分布。
- **看板可交互筛选**：点击看板里的城市 / 用工类型 / 学历标签，即按该条件**硬筛选**检索并自动提问；顶部出现可清除的筛选条（支持多条件 AND 组合）。
